		...
		Для ИндексМассива = 0 По ПозицииЧека.Количество() - 1 Цикл
			...
			Если ПозицияЧека.Свойство("ФискальнаяСтрока") Тогда
				...
				Если ПозицияЧека.Свойство("КонтрольнаяМарка") И НЕ ПустаяСтрока(ПозицияЧека.КонтрольнаяМарка) Тогда 
					Текст = Текст + ПостроитьПоле(НСтр("ru='[М]'"), ШиринаСтроки) + Символы.НПП;
				КонецЕсли;
			...
			ИначеЕсли ПозицияЧека.Свойство("ТекстоваяСтрока") Тогда
				Если ПозицияЧека.Свойство("ПереносСтроки") И ПозицияЧека.ПереносСтроки Тогда 
					ТекстовыеСтроки = МенеджерОборудованияКлиентСервер.ПостроитьПолеПереносом(ПозицияЧека.Текст, ШиринаСтроки);
				Иначе
					ТекстовыеСтроки = ПозицияЧека.Текст;
				КонецЕсли;
				Выравнивание = ?(ПозицияЧека.Свойство("Выравнивание"),  ПозицияЧека.Выравнивание, "Лево"); 
				Для НомерСтроки = 1 По СтрЧислоСтрок(ТекстовыеСтроки) Цикл
					...
					Текст = Текст + ВыделеннаяСтрока + Символы.ПС;
				КонецЦикла;
...
Функция КодПричиныИзмененияСведенийККТ(ПричинаИзмененияСведенийККТ) Экспорт
	...
	ПричиныИзмененияСведенийККТ.Вставить(ПредопределенноеЗначение("Перечисление.ПричиныИзмененияСведенийККТ.ВключениеРежимаАзартныхИгр")       , 18);
	ПричиныИзмененияСведенийККТ.Вставить(ПредопределенноеЗначение("Перечисление.ПричиныИзмененияСведенийККТ.ВыключениеРежимаАзартныхИгр")      , 17);
	ПричиныИзмененияСведенийККТ.Вставить(ПредопределенноеЗначение("Перечисление.ПричиныИзмененияСведенийККТ.ВключениеРежимаЛотереи")           , 20);
	ПричиныИзмененияСведенийККТ.Вставить(ПредопределенноеЗначение("Перечисление.ПричиныИзмененияСведенийККТ.ВыключениеРежимаЛотереи")          , 19);
...
// 
// Параметры:
//  ПланируемыйСтатусМаркируемогоТовара - Перечисление.ПланируемыйСтатусМаркируемогоТовара - Планируемый статус маркируемого товара.  
// 
// Возвращаемое значение:
...
// 
// Параметры:
//  ОтветОИСМОСтатусеТовара - Перечисление.ОтветОИСМОСтатусеТовара - Ответ от ИСМО.  
// 
// Возвращаемое значение:
...
// 
// Возвращаемое значение:
//  Перечисление.ОтветОИСМОСтатусеТовара - Ответ ОИСМ.
//
...
// 
// Параметры:
//  Текст - Строка - строки текста для отображения.
//  ЗначениеQRКода - Неопределено - Значение QRКода
// 
// Возвращаемое значение:
//  Структура - Параметры операции дисплей покупателя
//
Функция ПараметрыОперацииДисплейПокупателя(Текст = "", ЗначениеQRКода = Неопределено) Экспорт
	
	Результат = Новый Структура();
	Результат.Вставить("Текст", Текст); // Строка - Текстовая строка
...
Функция ПараметрыВыполненияЭквайринговойОперации() Экспорт;
	...
	Результат.Вставить("Сумма", 0); // Число,Обязательно - Сумма операции.     
	...
	Результат.Вставить("НомерСсылкиОперации"); // Строка,Необязательно - Уникальный код транзакции RRN.
