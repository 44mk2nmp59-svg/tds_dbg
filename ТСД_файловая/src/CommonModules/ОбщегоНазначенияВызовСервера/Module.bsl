...
Процедура ОбновитьПриложение() Экспорт
	...
	ПараметрыСеанса.СканерШтрихкодовИнициализирован = Ложь;
...
Процедура УстановитьРежимСинхронизацииДемоверсии() Экспорт
	...
	Если ЗначениеНастроекПовтИсп.ЭтоДемоверсия() Тогда
		Константы.РежимСинхронизацииДанных.Установить(Перечисления.РежимСинхронизацииДанных.Онлайн);
		Константы.ВидТранспортаСообщенийОбмена.Установить(Перечисления.ВидыТранспортаСообщенийОбмена.WS);
		Константы.АдресWS.Установить("http://orders:8096/Dn_test");
			
		//МенеджерЗаписи = РегистрыСведений.ЗначениеНастроекПользователя.СоздатьМенеджерЗаписи();
		//МенеджерЗаписи.Настройка = Перечисления.НастройкиПользователя.ВидАдресаПодключенияWS;
...
Функция ПолучитьСохраненныйТокен(ID_Устройства)Экспорт
	
	 ЗаписьТокен = РегистрыСведений.ТокенFCM.СоздатьМенеджерЗаписи();
	 ЗаписьТокен.ID_Устройства = ID_Устройства;
	 ЗаписьТокен.Прочитать();  

	 Возврат ЗаписьТокен.Токен;

КонецФункции

Процедура ЗаписатьНовыйТокен(ID_Устройства,Токен)Экспорт    
	
	ЗаписьТокен = РегистрыСведений.ТокенFCM.СоздатьМенеджерЗаписи();
	ЗаписьТокен.ID_Устройства = ID_Устройства;
	ЗаписьТокен.Токен = Токен;
	
	ЗаписьТокен.Записать();
