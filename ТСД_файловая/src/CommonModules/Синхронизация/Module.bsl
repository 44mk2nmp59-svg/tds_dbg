...
Функция ПолучитьИмяМакетаСхемы(ВерсияФормата) Экспорт
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru = 'Текущая версия приложения не поддерживает формат обмена %ВерсияФорматаОбмена%'");
...
Процедура ПроверитьУстановитьПодключение(Отказ, СообщениеОбОшибке, УстановитьПодключение = Ложь) Экспорт
	...
	Попытка
		...
		Если ЗначениеНастроекПовтИсп.ТранспортСинхронизацииЧерезWS() И ЗначениеНастроекВызовСервераПовтИсп.ЭтоАвтономныйРежим() Тогда
			...
			Если УстановитьПодключение И НЕ Отказ Тогда // проверка по ИДУстройства
				ЕстьСоединениеПоИД = WSПрокси.StartExchange(СтруктураНастроек.ВерсияПриложения,СтруктураНастроек.ИдентификаторМобильногоУстройства);
				
				Если НЕ ЕстьСоединениеПоИД.Success Тогда
					Если ЕстьСоединениеПоИД.DeleteAppData Тогда
						Отказ = Истина;
						СообщениеОбОшибке = ЕстьСоединениеПоИД.ResultMessage;
						ОбщегоНазначения.ОчиститьБД();

					Иначе
						КодОшибки = 21;
						Отказ = Истина;
						СообщениеОбОшибке = СообщенияОшибок[КодОшибки]+ Символы.ПС + ЕстьСоединениеПоИД.ResultMessage;
					КонецЕсли;
				Иначе
					УзелУстройста = ПланыОбмена.МобильноеПриложение.ЭтотУзел();
					Если УзелУстройста.Код <>  СтруктураНастроек.ИдентификаторМобильногоУстройства Тогда
						УзелУстройстаОбъект = УзелУстройста.ПолучитьОбъект();
						УзелУстройстаОбъект.Код  = СтруктураНастроек.ИдентификаторМобильногоУстройства;
						УзелУстройстаОбъект.Наименование = "Мобильная";
						УзелУстройстаОбъект.Записать();
					КонецЕсли;
					УзелЦентральный = ПланыОбмена.МобильноеПриложение.НайтиПоКоду("001");
					Если УзелЦентральный = ПланыОбмена.МобильноеПриложение.ПустаяСсылка() Тогда
						УзелЦентральныйОбъект = ПланыОбмена.МобильноеПриложение.СоздатьУзел();
						УзелЦентральныйОбъект.Код = "001";
						УзелЦентральныйОбъект.Наименование = "Центральная";
						УзелЦентральныйОбъект.Записать();
					КонецЕсли;
		...
		Иначе
			...
			Результат = ИнтеграцияТК_ВызовСервера.ПроверитьУстановитьПодключение();
			Если НЕ Результат.СоединениеУстановлено Тогда
				
				КодОшибки = 25;
				Отказ = Истина;
				СообщениеОбОшибке = СообщенияОшибок[КодОшибки]+ Символы.ПС + Результат.СообщениеОбОшибке;
				
			Иначе
				Если  Результат.Ответ.КодСостояния = 401 Тогда
					КодОшибки = 24;
					Отказ = Истина;
					СообщениеОбОшибке = СообщенияОшибок[КодОшибки]+ Символы.ПС + Результат.СообщениеОбОшибке;
				ИначеЕсли  Результат.Ответ.КодСостояния = 404 Тогда
					КодОшибки = 26;
					Отказ = Истина;
					СообщениеОбОшибке = СообщенияОшибок[КодОшибки]+ Символы.ПС + Результат.СообщениеОбОшибке;
	
				ИначеЕсли НЕ Результат.Ответ.КодСостояния = 200 Тогда
					КодОшибки = 23;
					Отказ = Истина;
					СообщениеОбОшибке = СообщенияОшибок[КодОшибки]+ Символы.ПС + Результат.СообщениеОбОшибке;
...
// GetPriceListPackage
//
Процедура ПолучитьПакетПрайсЛиста(ИдентификаторПередачи, Рестарт, ЗагрузкаЗавершена, Отказ, Лог, ЕстьИзменения = Ложь, ИнфоТекст = "") Экспорт
	...
	Попытка
		XMLТекстСообщения = WSПрокси.GetPriceListPackage(СтруктураНастроек.ИдентификаторМобильногоУстройства, ИдентификаторПередачи, Рестарт);
	Исключение
...
Функция ВозможностьЧтения(ТипДанныхXML)
	Если ТипДанныхXML = Неопределено Тогда
		Возврат Ложь;
	ИначеЕсли ТипДанныхXML.ИмяТипа = "CatalogObject.Номенклатура" ИЛИ ТипДанныхXML.ИмяТипа = "InformationBarCode" 
		ИЛИ ТипДанныхXML.ИмяТипа = "CatalogObject.Контрагенты" ИЛИ ТипДанныхXML.ИмяТипа = "CatalogObject.ТочкиДоставки" ИЛИ ТипДанныхXML.ИмяТипа = "InformationPrice" Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Процедура СоздатьОбновитьНоменклатуру(Данные)
	
	НоменклатураСсылка = Справочники.Номенклатура.ПолучитьСсылку(Новый УникальныйИдентификатор(Данные.Ref));
	...
	Если ЭтоНовый Тогда
		...
		Номенклатура.ПометкаУдаления = Данные.DeletionMark;
		Номенклатура.Код = Данные.Code;
		Номенклатура.Наименование = Данные.Description;
		Номенклатура.КоличествоЕденицЗаказа = Данные.КоличествоЕденицЗаказа;
		Номенклатура.КоличествоЕденицОсновной = Данные.КоличествоЕденицОсновной;
	...
	Иначе
		НужнаЗапись = Ложь;
		Если НЕ Номенклатура.ПометкаУдаления = Данные.DeletionMark Тогда
			Номенклатура.ПометкаУдаления = Данные.DeletionMark;
			НужнаЗапись = Истина;
		КонецЕсли;
		
		Если НЕ Номенклатура.Код = Данные.Code Тогда
			Номенклатура.Код = Данные.Code;
			НужнаЗапись = Истина;
		КонецЕсли;

		Если НЕ Номенклатура.Наименование = Данные.Description Тогда
			Номенклатура.Наименование = Данные.Description;
			НужнаЗапись = Истина;
		КонецЕсли;
		
		Если НЕ Номенклатура.КоличествоЕденицЗаказа = Данные.КоличествоЕденицЗаказа Тогда
			Номенклатура.КоличествоЕденицЗаказа = Данные.КоличествоЕденицЗаказа;
			НужнаЗапись = Истина;
		КонецЕсли;
		
		Если НЕ Номенклатура.КоличествоЕденицОсновной = Данные.КоличествоЕденицОсновной Тогда
			Номенклатура.КоличествоЕденицОсновной = Данные.КоличествоЕденицОсновной;
			НужнаЗапись = Истина;
		КонецЕсли;
...
Процедура ЗаписатьШтрихкодНоменклатуры(Данные)
	...
	Для Каждого ТекСтрока Из Данные.ListBarCod.List Цикл 
		Запись = НаборЗаписей.Добавить();
		Запись.Номенклатура = НоменклатураСсылка;
		Запись.Штрихкод = СокрЛП(ТекСтрока.BarCode);
		Запись.Коэффициент = ТекСтрока.Cof;
	КонецЦикла;
...
Процедура ЗаписатьЦеныНоменклатуры(Данные)
	
	НоменклатураСсылка = Справочники.Номенклатура.ПолучитьСсылку(Новый УникальныйИдентификатор(Данные.Goods));
	НаборЗаписей = РегистрыСведений.БуферПодбораТоваров.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Ссылка.Установить(НоменклатураСсылка);
	НаборЗаписей.Прочитать();
	НаборЗаписей.Очистить();
	
	Запись = НаборЗаписей.Добавить();
	Запись.Ссылка = НоменклатураСсылка;
	Запись.Цена = Данные.Price;
	
	НаборЗаписей.Записать(Истина);
		
КонецПроцедуры

Процедура СоздатьОбновитьКонтрагента(Данные)
	
	КонтрагентСсылка = Справочники.Контрагенты.ПолучитьСсылку(Новый УникальныйИдентификатор(Данные.Ref));
	...
	Если ЭтоНовый Тогда
		...
		Контрагент.ПометкаУдаления = Данные.DeletionMark;
		Контрагент.Код = Данные.Code;
		Контрагент.Наименование = Данные.Description;
	...
	Иначе
		НужнаЗапись = Ложь;
		Если НЕ Контрагент.ПометкаУдаления = Данные.DeletionMark Тогда
			Контрагент.ПометкаУдаления = Данные.DeletionMark;
			НужнаЗапись = Истина;
		КонецЕсли;
		
		Если НЕ Контрагент.Код = Данные.Code Тогда
			Контрагент.Код = Данные.Code;
			НужнаЗапись = Истина;
		КонецЕсли;
		
		Если НЕ Контрагент.Наименование = Данные.Description Тогда
			Контрагент.Наименование = Данные.Description;
			НужнаЗапись = Истина;
		КонецЕсли;
	...
	Для Каждого ТекСтрокаДоговор Из Данные.ListDogovor.List Цикл 
		
		ДоговорСсылка = Справочники.ДоговорыВзаиморасчетов.ПолучитьСсылку(Новый УникальныйИдентификатор(ТекСтрокаДоговор.Ref));
		...
		Если Договор = Неопределено Тогда
			...
			Договор.УстановитьСсылкуНового(ДоговорСсылка);
		КонецЕсли;
		
		
		Если ЭтоНовый Тогда
			
			Договор.ПометкаУдаления = ТекСтрокаДоговор.DeletionMark;
			//Договор.Код = ТекСтрокаДоговор.Code;
			Договор.Наименование = ТекСтрокаДоговор.Description;
			Договор.Контрагент = КонтрагентСсылка;
		...
		Иначе
			...
			НужнаЗапись = Ложь;
			Если НЕ Договор.ПометкаУдаления = ТекСтрокаДоговор.DeletionMark Тогда
				Договор.ПометкаУдаления = ТекСтрокаДоговор.DeletionMark;
				НужнаЗапись = Истина;
			КонецЕсли;
			//
			//Если НЕ Договор.Код = ТекСтрокаДоговор.Code Тогда
			//	Договор.Код = ТекСтрокаДоговор.Code;
			//	НужнаЗапись = Истина;
			//КонецЕсли;
			
			Если НЕ Договор.Наименование = ТекСтрокаДоговор.Description Тогда
				Договор.Наименование = ТекСтрокаДоговор.Description;
				НужнаЗапись = Истина;
			КонецЕсли;
			
			Если НЕ Договор.Контрагент = КонтрагентСсылка Тогда
				Договор.Контрагент = КонтрагентСсылка;
				НужнаЗапись = Истина;
			КонецЕсли;
...
Процедура СоздатьОбновитьАдресДоставки(Данные)
	
	АдресДоставкиСсылка = Справочники.АдресДоставки.ПолучитьСсылку(Новый УникальныйИдентификатор(Данные.Ref));
	ВладелецСсылка      = Справочники.Контрагенты.ПолучитьСсылку(Новый УникальныйИдентификатор(Данные.Owner));

	АдресДоставки = АдресДоставкиСсылка.ПолучитьОбъект();
	ЭтоНовый = Ложь;
	
	Если АдресДоставки = Неопределено Тогда
		ЭтоНовый = Истина;
		АдресДоставки = Справочники.АдресДоставки.СоздатьЭлемент();
		АдресДоставки.УстановитьСсылкуНового(АдресДоставкиСсылка);
	КонецЕсли;
	
	
	Если ЭтоНовый Тогда
		
		АдресДоставки.ПометкаУдаления = Данные.DeletionMark;
		//АдресДоставки.Код = Данные.Code;
		АдресДоставки.Наименование = Данные.Description;
		АдресДоставки.Владелец     = ВладелецСсылка; 
		
		АдресДоставки.ОбменДанными.Загрузка = Истина;
		АдресДоставки.Записать();
		
	Иначе
		НужнаЗапись = Ложь;
		Если НЕ АдресДоставки.ПометкаУдаления = Данные.DeletionMark Тогда
			АдресДоставки.ПометкаУдаления = Данные.DeletionMark;
			НужнаЗапись = Истина;
		КонецЕсли;
		
		Если НЕ АдресДоставки.Наименование = Данные.Description Тогда
			АдресДоставки.Наименование = Данные.Description;
			НужнаЗапись = Истина;
		КонецЕсли;
		
		Если НЕ АдресДоставки.Владелец = ВладелецСсылка Тогда
			АдресДоставки.Владелец = ВладелецСсылка;
			НужнаЗапись = Истина;
		КонецЕсли;
...
Процедура Загрузка(Отказ, Лог, ЕстьИзменения = Ложь, ИнфоТекст = "") Экспорт
	
	ПродолжитьОбмен = Истина;
	Попытка
		...
		Пока ПродолжитьОбмен Цикл 
			
			СообщениеОбмена = СформироватьСообщениеОбмена(ВерсияФорматаОбмена,Фабрика);
			
			Сообщения = WSПрокси.GetCatalogs(СтруктураНастроек.ИдентификаторМобильногоУстройства,СообщениеОбмена);
			
			Если Сообщения.Success Тогда
				Попытка
					ЧтениеXML = Новый ЧтениеXML;
					ЧтениеXML.УстановитьСтроку(Сообщения.ResultMessage.Получить());
					...
					Пока ВозможностьЧтения(ТипДанныхXML)Цикл 
						
					
						Тип =  Фабрика.Тип("http://www.digma.ua/Mobile/Exchange", ТипДанныхXML.ИмяТипа);
						Данные = Фабрика.ПрочитатьXML(ЧтениеXML,Тип);
						Если  ТипДанныхXML.ИмяТипа = "CatalogObject.Номенклатура" Тогда
							СоздатьОбновитьНоменклатуру(Данные);
						ИначеЕсли ТипДанныхXML.ИмяТипа = "InformationBarCode" Тогда
							ЗаписатьШтрихкодНоменклатуры(Данные);
						ИначеЕсли ТипДанныхXML.ИмяТипа = "CatalogObject.Контрагенты" Тогда
							СоздатьОбновитьКонтрагента(Данные);
						ИначеЕсли ТипДанныхXML.ИмяТипа = "CatalogObject.ТочкиДоставки" Тогда	
							СоздатьОбновитьАдресДоставки(Данные);
						ИначеЕсли ТипДанныхXML.ИмяТипа = "InformationPrice" Тогда
							 ЗаписатьЦеныНоменклатуры(Данные);
				...
				ПродолжитьОбмен = Сообщения.Continue;
			Иначе
...
Процедура Выгрузка(Отказ, Лог, ЕстьИзменения = Ложь, ИнфоТекст = "") Экспорт
	
	Попытка
		
		ТекстСообщения ="";
		ДокументыДляВыгрузки = Документы.ЗаказКлиента.ЗаказыКВыгрузке();
		...
		Если ЗначениеНастроекПовтИсп.ТранспортСинхронизацииЧерезWS() Тогда
			...
			Для Каждого ДокСсылка Из ДокументыДляВыгрузки Цикл 
				
				ДокументXDTO = ПолучитьДокументЗаказ(ВерсияФорматаОбмена, ДокСсылка, Фабрика, Отказ, Лог);
				
				ТекстXML = ПолучитьТекстXMLОбъектаXDTO(Фабрика, ДокументXDTO, Отказ, Лог);
				
				СообщенияОтвет = WSПрокси.PostDocs(СтруктураНастроек.ИдентификаторМобильногоУстройства, "Order", ТекстXML);
				
				Если СообщенияОтвет.Success Тогда
					
					ДокументОбъект = ДокСсылка.ПолучитьОбъект();
					ДокументОбъект.ОбменДанными.Загрузка = Истина;
					ДокументОбъект.СтатусЗаказа = Перечисления.СтатусыЗаказовКлиентов.Отправлен;
					ДокументОбъект.Записать(РежимЗаписиДокумента.Запись);
				Иначе
					
					Отказ = Истина;
					
					ОбщегоНазначенияКлиентСервер.ДополнитьТекст(Лог, СообщенияОтвет.ResultMessage);
...
Функция СформироватьСообщениеОбмена(ВерсияФорматаОбмена,Фабрика)
	СообщениеXDTO = ЗначениеXDTO(ВерсияФорматаОбмена, Фабрика, "MessageExchange");
	СообщениеXDTO.Success = Истина;
	СообщениеXDTO.Continue = Ложь;
	
	ЗаписьХМЛ = Новый ЗаписьXML;
	ЗаписьХМЛ.УстановитьСтроку();
	Записьсообщения = ПланыОбмена.СоздатьЗаписьСообщения();
	Записьсообщения.НачатьЗапись(ЗаписьХМЛ,СинхронизацияПовтИсп.УзелЦентральной());
	Записьсообщения.ЗакончитьЗапись();
	
	РезультатЗаписи = ЗаписьХМЛ.Закрыть();
	СообщениеXDTO.ResultMessage = Новый ХранилищеЗначения(РезультатЗаписи,Новый СжатиеДанных(9));
	
	Возврат СообщениеXDTO;
	
КонецФункции

Функция ПолучитьДокументЗаказ(ВерсияФорматаОбмена, ВыгружаемыйДокумент, Фабрика, Отказ, Лог)
	...
	|	ЗаказКлиента.РеглУчет КАК РеглУчет,
	|	ЗаказКлиента.Контрагент КАК Контрагент,
	|	ЗаказКлиента.Договор КАК Договор,
	|	ЗаказКлиента.АдресДоставки КАК АдресДоставки,
	|	ЗаказКлиента.ЖелаемаяДатаПокупки КАК ЖелаемаяДатаПокупки,
	...
	ДокументXDTO = ЗначениеXDTO(ВерсияФорматаОбмена, Фабрика, "Order");
	
	
	Пока Выборка.Следующий() Цикл
		
		ДокументXDTO.UUID     = Строка(Выборка.Ссылка.УникальныйИдентификатор());
		ДокументXDTO.Number   = СокрЛП(Выборка.Номер);
		ДокументXDTO.Date     = Выборка.Дата;
		ДокументXDTO.Regl     = Выборка.РеглУчет;
		ДокументXDTO.Buyer    = Строка(Выборка.Контрагент.УникальныйИдентификатор());
		ДокументXDTO.Contract = Строка(Выборка.Договор.УникальныйИдентификатор());
		ДокументXDTO.Address  = Строка(Выборка.АдресДоставки.УникальныйИдентификатор());
		ДокументXDTO.Comment  = СокрЛП(Выборка.Комментарий);
		ДокументXDTO.DeliveryDate = Выборка.ЖелаемаяДатаПокупки;
		
		ТипСтроки = ДокументXDTO.Свойства().Получить("ListGoods").Тип;
		ДобавляемыеСтроки	 = Фабрика.Создать(ТипСтроки);
		
		ДобавляемаяСтрокаТип  = ДобавляемыеСтроки.Свойства().Получить("List").Тип;
		
		
		ВыборкаТовары = Выборка.Товары.Выбрать();
		Пока ВыборкаТовары.Следующий() Цикл 
			
			ДобавляемаяСтрока     = Фабрика.Создать(ДобавляемаяСтрокаТип);
			ДобавляемаяСтрока.Ref      = Строка(ВыборкаТовары.Номенклатура.УникальныйИдентификатор());
			ДобавляемаяСтрока.Quantity = ВыборкаТовары.Количество;
			ДобавляемаяСтрока.Price = ВыборкаТовары.Цена;

			ДобавляемыеСтроки.List.Добавить(ДобавляемаяСтрока);
			
		КонецЦикла;
		ДокументXDTO.ListGoods = ДобавляемыеСтроки;
...
Процедура ОбработатьОтветОтправкиДокумента(Отказ, XMLТекстСообщения, Лог)
	...
	Если (НЕ Данные.Свойства().Получить("IsFormed") = Неопределено
		...
		(НЕ Данные.Свойства().Получить("Succeeded") = Неопределено
			И (Данные.Succeeded = "false" ИЛИ Данные.Succeeded = Ложь)) Тогда
...
Функция ПолучитьОтчетОПродажах(ВерсияФорматаОбмена, ОтчетыДляВыгрузки, Фабрика, Отказ, Лог, ПолучитьОбъектXDTO = Ложь, ИнфоТекст = "")
	...
	ОтчетыОПродажахXDTO = ЗначениеXDTO(ВерсияФорматаОбмена, Фабрика, "Order");
...
Процедура ВыгрузитьОтчет100Х(ВерсияФорматаОбмена, Фабрика, Отчет, ОтчетыОПродажахXDTO)
	...
	Если Отчет.СуммаДокумента = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ОтчетОРозничныхПродажахТовары.Номенклатура.КодПоиска КАК Code,
	|	ОтчетОРозничныхПродажахТовары.Цена КАК Price,
	|	ОтчетОРозничныхПродажахТовары.Количество КАК Quantity,
	|	ОтчетОРозничныхПродажахТовары.Сумма КАК Amount
	|ИЗ
	...
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА ОтчетОРозничныхПродажахОплаты.ТипОплаты = ЗНАЧЕНИЕ(Перечисление.ТипыОплаты.Наличные)
	|			ТОГДА 0
	|		ИНАЧЕ 1
	|	КОНЕЦ КАК PaymentType,
	|	ОтчетОРозничныхПродажахОплаты.Сумма КАК Amount,
	|	ЕСТЬNULL(ОтчетОРозничныхПродажахОплаты.ВидОплаты.КодПоиска, &ПустаяСтрока) КАК CodePaymentSort,
	|	ЕСТЬNULL(ОтчетОРозничныхПродажахОплаты.ВидОплаты.КодПоиска, &ПустаяСтрока) КАК CodePaymentKind
	|ИЗ
	|	Документ.ОтчетОРозничныхПродажах.Оплаты КАК ОтчетОРозничныхПродажахОплаты
	|ГДЕ
	|	ОтчетОРозничныхПродажахОплаты.Ссылка = &Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ДокКассоваяСмена.НачалоКассовойСмены,
	|	ДокКассоваяСмена.ОкончаниеКассовойСмены
	|ИЗ
	...
	РезультатСведенияОСмене = РезультатЗапроса[2];
	...
	ОтчетXDTO.SessionNumber = Число(ВыборкаСведенияОСмене.Номер);
	...
	// Товары
	РезультатТовары = РезультатЗапроса[0];
	...
	// Оплаты
	РезультатОплаты = РезультатЗапроса[1];
...
Процедура ВыгрузитьОтчет2000(ВерсияФорматаОбмена, Фабрика, ОтчетыДляВыгрузки, ОтчетыОПродажахXDTO)
	...
	|	Чек.СистемаНалогообложения КАК СистемаНалогообложения,
	...
	|			И ОтчетыОПРодажах.ОтчетОПродажах = Чек.ОтчетОРозничныхПродажах
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Чек
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОтчетыОПРодажах.ОтчетОПродажах,
	|	ОтчетыОПРодажах.КассоваяСмена,
	|	ОтчетыОПРодажах.НачалоКассовойСмены,
	|	ОтчетыОПРодажах.ОкончаниеКассовойСмены,
	|	ОтчетыОПРодажах.Номер,
	|	ОтчетыОПРодажах.Дата
	|ИЗ
	|	ОтчетыОПродажах КАК ОтчетыОПРодажах
	|;
	|
	...
	|	Чеки.Номер,
	|	Чеки.Дата,
	|	Чеки.ТипРасчетаДенежнымиСредствами,
	|	Чеки.СистемаНалогообложения,
	|	Чеки.Заказ,
	|	Чеки.КассоваяСмена,
	|	Чеки.ОтчетОПродажах,
	|	Чеки.ЧекПродажи
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.Чек.АкцизныеМарки КАК ЧекАкцизныеМарки
	|		ПО ЧекТовары.Ссылка = ЧекАкцизныеМарки.Ссылка
	|			И ЧекТовары.КлючСвязи = ЧекАкцизныеМарки.КлючСвязи
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВнесениеИзъятиеНаличных.КассоваяСмена,
	|	ВнесениеИзъятиеНаличных.Номер,
	|	ВнесениеИзъятиеНаличных.Дата
	|ИЗ
	...
	Для Каждого ЗаписьОтчетОПродажах Из ОтчетыОПРодажах Цикл
		...
		Для Каждого ЗаписьЧек Из НайденныеЧеки Цикл
			...
			ЧекXDTO.TaxationSystem = ОбщегоНазначенияПовтИсп.ПолучитьКодТипаСистемыНалогообложенияККТ(
				ОбщегоНазначенияПовтИсп.ПолучитьСистемуНалогообложенияККТ(ЗаписьЧек.СистемаНалогообложения)
			);
			...
			ЧекXDTO.OperationType = МенеджерОборудованияКлиентСервер.ПолучитьКодТипаРасчетаДенежнымиСредствами(
			...
			Для Каждого ЗаписьТовар Из НайденныеТовары Цикл
				...
				Если ЗначениеЗаполнено(ЗаписьТовар.КодАкцизнойМарки) Тогда
					ЗаписьТоварXDTO.AlcoholExciseStamp = ЗаписьТовар.КодАкцизнойМарки;
				КонецЕсли;
				
				ЗаписьТоварXDTO.TaxRate = ОбщегоНазначенияПовтИсп.ПолучитьСтавкуНДСККТ(ЗаписьТовар.СтавкаНДС);
				
				//! МенеджерОборудованияКлиентСервер.ПолучитьКодПризнакСпособаРасчета(Перечисления.ПризнакиСпособаРасчета.ПередачаСПолнойОплатой);
				//! до появления авансов
				ЗаписьТоварXDTO.PaymentMethod = 4;
			...
			Если НЕ НайденныеОплаты.Количество() = 0 Тогда
				...
				Для Каждого ЗаписьОплата Из НайденныеОплаты Цикл
					...
					Если ЗаписьОплата.ТипОплаты = Перечисления.ТипыОплаты.Наличные Тогда
						...
						ЗаписьОплатаXDTO.Cash = ЗаписьОплата.Сумма;
					Иначе
						
						ЗаписьОплатаXDTO.ElectronicPayment = ЗаписьОплата.Сумма;
					КонецЕсли;
		...
		Для Каждого Order Из Данные.Order Цикл
			...
			Если НЕ СвойстваXDTO.Получить("LastName") = Неопределено Тогда
				...
				Если ЗначениеЗаполнено(Order.LastName) Тогда
					...
					Если НЕ ЗаказОбъект.Покупатель = Order.LastName Тогда
						
						ЗаказОбъект.Покупатель = Order.LastName;
			...
			Если НЕ СвойстваXDTO.Получить("FirstName") = Неопределено Тогда
				...
				Если ЗначениеЗаполнено(Order.FirstName) Тогда
					...
					Если НЕ ЗаказОбъект.Покупатель = Order.FirstName Тогда
						
						ЗаказОбъект.Покупатель = ЗаказОбъект.Покупатель + " " + Order.FirstName;
						ЕстьИзмененияЭлемента = Истина;
					КонецЕсли;
...
Процедура ЗаполнитьРеквизитыНоменклатуры2000(ТоварXDTO, КодПоиска, Наименование, Цена, Штрихкоды, ЕстьИзменения)
	...
	// ЕдиницаИзмерения
	ЕдиницаИзмерения = ПолучитьЕдиницуИзмерения(ТоварXDTO.UnitCode, ТоварXDTO.UnitUUID);
...
Функция ПолучитьЕдиницуИзмерения(КодПоиска, УУИД = Неопределено)
...
Процедура ЗагрузитьЕдиницыИзмерения(ЕдиницыИзмеренияXDTO, ЕстьИзменения)
	...
	Для Каждого ЕдиницаИзмеренияXDTO Из ЕдиницыИзмеренияXDTO.Unit Цикл
		...
		ЕдиницаИзмеренияСсылка = ПолучитьЕдиницуИзмерения(ЕдиницаИзмеренияXDTO.Code, УУИД);
...
Функция ПолучитьWSПрокси(Отказ, СтруктураНастроек, СтрокаСообщенияОбОшибке = "", СообщениеПользователю = "", Таймаут = 180)
	...
	СтруктураНастроек.Вставить("WSURLПространстваИменСервиса", "http://www.digma.ua/Mobile/Exchange");
	СтруктураНастроек.Вставить("WSИмяСервиса",                 "MobileExchange");
