...
Функция СоздатьГруппуЭлементовОтбора(КоллекцияЭлементов, Представление, ТипГруппы) Экспорт
	...
	ГруппаЭлементовОтбора.ПредставлениеЗаказа    = Представление;
...
Процедура УстановитьПорядокСписка(Список, ПолеСортировки, НаправлениеСортировки) Экспорт
	
	КоллекцияЭлементов = Список.Порядок.Элементы;
	КоллекцияЭлементов.Очистить();
	
	ЭлементПорядка = КоллекцияЭлементов.Добавить(Тип("ЭлементПорядкаКомпоновкиДанных"));
	ЭлементПорядка.Поле = Новый ПолеКомпоновкиДанных(ПолеСортировки);
	ЭлементПорядка.ТипУпорядочивания = НаправлениеСортировки;
...
КонецПроцедуры
 ...
 Процедура ОформитьКоличественныеПоляВвода(Форма, СписокПолей,Весовой) Экспорт
	...
	МассивПолей = РазложитьСтрокуВМассивПодстрок(СписокПолей,,,Истина);
	ФорматПоляВесовой = "ЧЦ=15; ЧДЦ=3; ЧРД=.; ЧН=0.000; ЧГ=0";
	ФорматПоля = "ЧЦ=15; ЧДЦ=0; ЧРД=.; ЧН=0; ЧГ=0";
...
Процедура УстановитьПометкуКнопки(Кнопка) Экспорт
	
	Кнопка.Пометка = Истина;
	Кнопка.Картинка = БиблиотекаКартинок.ЗаписатьИЗакрыть;
	
КонецПроцедуры

Процедура СнятьПометкуКнопки(Кнопка) Экспорт
...
Функция СоздатьТабличныйДокументСТекстом(Текст) Экспорт
	
	ТабДок = Новый ТабличныйДокумент;
	
	ТабДок.ТекущаяОбласть.Текст = Текст;
	
	ШиринаА4 = 100;
	ТабДок.ТекущаяОбласть.ШиринаКолонки = ШиринаА4;
...
Функция СкопироватьСписокЗначений(СписокИсточник) Экспорт
	...
	Для Каждого ЭлементСписка Из СписокИсточник Цикл
		СписокРезультат.Добавить(
			...
			ЭлементСписка.ПредставлениеЗаказа, 
...
Функция ДанныеНомераТелефонаКорректны(НомерТелефона) Экспорт
...
Функция НайтиЭлементОтбораПоПредставлению(КоллекцияЭлементов, Представление)
	...
	Для каждого ЭлементОтбора Из КоллекцияЭлементов Цикл
		Если ЭлементОтбора.ПредставлениеЗаказа = Представление Тогда
...
Процедура УдалитьГруппуОтбораКоллекции(КоллекцияЭлементов, ПредставлениеГруппы)
	...
	Для Каждого ЭлементОтбора Из КоллекцияЭлементов Цикл
		Если ТипЗнч(ЭлементОтбора) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных")
			И ЭлементОтбора.ПредставлениеЗаказа = ПредставлениеГруппы Тогда
			КоллекцияЭлементов.Удалить(ЭлементОтбора);
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьТекстДатаФормированияОтчета(ДатаФормирования) Экспорт
	
	Текст = НСтр("ru = 'Дата формирования отчета: %ДатаФормирования%'");
	Текст = СтрЗаменить(Текст, "%ДатаФормирования%", ДатаФормирования);
...
Процедура ОчиститьСотрудника()Экспорт
	ОбщегоНазначенияВызовСервера.УстановитьПараметрСеанса("Сотрудник","");
	ОбщегоНазначенияВызовСервера.УстановитьПараметрСеанса("Сотрудник_ID","");
	
КонецПроцедуры


Функция СтруктураДанныеВРаботе()Экспорт
	стСтруктура = Новый Структура("ДокументUUID,ТипДокумента,Артикул,МРЦ,Количество,Остаток,СканированиеТовар,СканированиеЦенник,НаименованиеТовара,НаименованиеЕдиницы,Коэффициент,ИспМРЦ,Пересчет,ПревышенОстаток,КоличествоРасхождение,КоличествоВведено,ПричинаРасхожденияПорядок,ПричинаРасхожденияСтрока,КоличествоПоставщика,ПричинаРасхождения");
	Возврат стСтруктура;
КонецФункции
