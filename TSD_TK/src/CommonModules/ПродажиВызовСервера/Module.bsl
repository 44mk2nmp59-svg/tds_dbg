...
Функция ОткрытьДокументКассоваяСмена() Экспорт
...
Процедура ЗакрытьДокументКассоваяСмена() Экспорт
    ...
    ЗапросПоОплате.УстановитьПараметр("КассоваяСмена", КассоваяСмена);
    ЗапросПоОплате.УстановитьПараметр("ВидОперацииПродажа", 0);

    ОтменитьТранзакцию = Ложь;
    НачатьТранзакцию();

    РезультатЗапросаПоЧекам           = ЗапросПоЧекам.Выполнить();
    РезультатЗапросПоТоварам          = ЗапросПоТоварам.Выполнить();
    РезультатЗапросаПоВозвратам       = ЗапросПоВозвратам.Выполнить();
    РезультатЗапросаПоОплате          = ЗапросПоОплате.Выполнить();

    ТаблицаОплаты        = РезультатЗапросаПоОплате.Выгрузить();

    Если Не (РезультатЗапросПоТоварам.Пустой() И РезультатЗапросаПоВозвратам.Пустой()) Тогда
        ...
        Выборка = РезультатЗапросПоТоварам.Выбрать();
        ...
        ОтчетОРозничныхПродажах.СуммаДокумента = ОтчетОРозничныхПродажах.Товары.Итог("Сумма");
        ...
        ОтчетОРозничныхПродажах.СуммаВозвратов = ОтчетОРозничныхПродажах.ВозвращенныеТовары.Итог("Сумма");
        ...
        Если Не ОтменитьТранзакцию И ЕстьПродажиВозвратыТовара Тогда

            Если ЕстьПродажиВозвратыТовара Тогда

                Для Каждого СтрокаОплаты Из ТаблицаОплаты Цикл
                    НоваяСтрока = ОтчетОРозничныхПродажах.Оплаты.Добавить();
            ...
            Попытка
                ...
                Если Не ОтменитьТранзакцию Тогда
                    ...
                    Исключение
                        ...
                        Текст = СтрЗаменить(НСтр("ru = 'Не удалось заархивировать чеки ККМ! %ОписаниеОшибки%'"),
            ...
            Исключение
                ...
                Текст = СтрЗаменить(НСтр(
                    "ru = 'Не удалось записать документ ""Отчет ККМ о продажах!""%ОписаниеОшибки%'"),
                    "%ОписаниеОшибки%", ОписаниеОшибки());
                ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Текст);
        ...
        Иначе
            ...
            Для Каждого ДокументКПроведению Из МассивДокументовКПроведению Цикл
                Попытка
                    ...
                    Если РезультатПроверки Тогда
                        ...
                        Если Не ДокументКПроведению.Проведен Тогда
                            СписокНеПроведенныхДокументов.Добавить(ДокументКПроведению.Ссылка,
                                ДокументКПроведению.Ссылка.Метаданные().Имя);
                        КонецЕсли;
                    Иначе
                ...
                Исключение
                    ...
                    Текст = СтрЗаменить(НСтр("ru = 'Не удалось провести документ ""%Документ%"".'"), "%Документ%",
    ...
    Иначе
		...
		// Не было продаж и возвратов. Закрываем смену без отчета о продажах
        КассоваяСменаОбъект = КассоваяСмена.ПолучитьОбъект();
        КассоваяСменаОбъект.ОкончаниеКассовойСмены = ОбщегоНазначенияКлиентСервер.ПолучитьТекущуюДату();
        КассоваяСменаОбъект.Закрыта = Истина;
        КассоваяСменаОбъект.Записать(РежимЗаписиДокумента.Проведение);
...
Функция СформироватьТекстXОтчетаДляПечати(КассоваяСмена) Экспорт
    ...
    ТекстXОтчета = СтрЗаменить(ТекстXОтчета, "%ПродажаНаличными%", Формат(ДанныеОтчета.ПродажаНаличными,
        ФорматСуммовыхПолей));
    ТекстXОтчета = СтрЗаменить(ТекстXОтчета, "%ПродажаПлатежнойКартой%", Формат(ДанныеОтчета.ПродажаПлатежнойКартой,
        ФорматСуммовыхПолей));

    ТекстXОтчета = СтрЗаменить(ТекстXОтчета, "%Возврат%", Формат(ДанныеОтчета.Возврат, ФорматСуммовыхПолей));
    ТекстXОтчета = СтрЗаменить(ТекстXОтчета, "%ВозвратНаличными%", Формат(ДанныеОтчета.ВозвратНаличными,
        ФорматСуммовыхПолей));
    ТекстXОтчета = СтрЗаменить(ТекстXОтчета, "%ВозвратПлатежнойКартой%", Формат(ДанныеОтчета.ВозвратПлатежнойКартой,
        ФорматСуммовыхПолей));
    ...
    ТекстXОтчета = СтрЗаменить(ТекстXОтчета, "%КоличествоПродаж%", Формат(КоличествоОпераций.КоличествоПродаж,
        ФорматКоличества));
    ТекстXОтчета = СтрЗаменить(ТекстXОтчета, "%КоличествоВозвратов%", Формат(КоличествоОпераций.КоличествоВозвратов,
        ФорматКоличества));
    ТекстXОтчета = СтрЗаменить(ТекстXОтчета, "%КоличествоВнесений%", Формат(КоличествоОпераций.КоличествоВнесений,
        ФорматКоличества));
    ТекстXОтчета = СтрЗаменить(ТекстXОтчета, "%КоличествоВыплат%", Формат(КоличествоОпераций.КоличествоВыплат,
        ФорматКоличества));
...
Функция ПолучитьНомерСменыДляПечати(КассоваяСмена) Экспорт

    Возврат ОбщегоНазначенияКлиентСервер.ПредставлениеНомера(КассоваяСмена.Номер, Ложь);
...
Процедура УдалитьАннулированныеЧеки(ОтменитьТранзакцию)
    ...
    Если Не ОтменитьТранзакцию Тогда
        Попытка
            МассивСсылокАннулированныеЧеки = РезультатЗапросаПоАннулированнымЧекам.Выгрузить().ВыгрузитьКолонку(
                "ОтложенныйЧек");
        ...
        Исключение
            ...
            Текст = СтрЗаменить(НСтр("ru = 'Не удалось удалить аннулированные чеки. %ОписаниеОшибки%'"),
                "%ОписаниеОшибки%", ОписаниеОшибки());
            ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Текст);
