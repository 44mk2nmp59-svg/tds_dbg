...
Функция РазобратьШтриховойКодТовара(Знач Штрихкод, Знач ШтрихкодВBase64 = Ложь) Экспорт;
	#Если ВебКлиент Тогда
		// В случае работы ВебКлиент принудительно выполняем на сервере. 
...
Функция РазобратьШтриховойКодТовараСлужебная(Знач Штрихкод, Знач ШтрихкодВBase64 = Ложь) Экспорт;
	...
	Если ДлинаШтрихкода = 20 Тогда // Код товара средства идентификации мехового изделия.
		...
		// С учетом приказа ФНС России от 12.04.2023 N ЕД-7-20/239@ 
		// проверяется формат содержания считанной последовательности на шаблон СС-ССССС-СССССССССС.
		Если Сред(ШтриховойКодТовара, 3, 1) = "-" И Сред(ШтриховойКодТовара, 10, 1) = "-" Тогда
			...
			Если ПроверкаСтроки(СимвольныйКодГосударства, ПараметрыПроверкиСтроки(Истина)) 
				И ПроверкаСтроки(ТипИдентификационногоЗнака, ПараметрыПроверкиСтроки(Истина, Ложь, Истина)) 
				И ПроверкаСтроки(СерияИдентификационногоЗнака, ПараметрыПроверкиСтроки(Истина, Ложь, Истина)) Тогда
...
Функция СформироватьТехническийКодДляНечитаемогоDataMatrixВBase64(GTIN, СерийныйНомер, КриптоЧасть = "0000") Экспорт
	...
	ТехническийКод = СтрЗаменить(ТехническийКод, "<GS1>", РазделительGS1());
...
Функция КодыGS1() Экспорт
	...
	Возврат МенеджерОборудованияМаркировкаКлиентСерверПовтИсп.КодыGS1();
